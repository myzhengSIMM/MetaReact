...
def augment_smiles_file(input_file_path, output_file_path, augmentation_factor=9):
    ...
    # 数据增强
    augmented_data = {'substrate_smiles': []}
    for _, row in df.iterrows():
        substrate = row['substrate_smiles'].strip()

        
        augmented_data['substrate_smiles'].extend(augment_smiles(substrate, sme, augmentation_factor))
...
# for type in ['train','val','test']:
    # 指定输入和输出文件
input_file_path = "predict_add_true_drugbank_47_51.csv"
output_file_path = "test_argue10_group_new.tsv"
